...
def set(data):
    ...
    # 打印接收到的数据（用于调试）
    print(f"{Colors.GREEN.value}Received data: serial_id={serial_id}, merchant_name = {merchant_name}, x1={x1}, y1={y1}, x2={x2}, y2={y2}, state={state}{Colors.END.value}")   
    # 存入redis
    redis_util.hset(SERIAL_ID_MAP, serial_id, merchant_name)
    redis_util.hmset(f'{serial_id}:{DOOR_STATE}', {
        'x1': x1,'y1': y1, 'x2': x2, 'y2': y2, 'state': state
    })

    scheduler_thread = threading.Thread(target=schedule_thread, args=(serial_id), daemon=True)
